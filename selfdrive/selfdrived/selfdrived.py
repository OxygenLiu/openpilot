...
class SelfdriveD:
  ...
  def update_events(self, CS):
    ...
    # Non-critical processes: crash is logged but does NOT block openpilot engagement
    optional_procs = {'mapd', 'speedlimitd'}
    not_running = {p.name for p in self.sm['managerState'].processes if not p.running and p.shouldBeRunning}
    optional_down = not_running & optional_procs
    not_running -= optional_procs
    if self.sm.recv_frame['managerState'] and optional_down:
      if optional_down != getattr(self, '_optional_down_prev', set()):
        cloudlog.warning("optional process not running: %s", ', '.join(optional_down))
      self._optional_down_prev = optional_down
      self.events.add(EventName.mapdProcessError)
