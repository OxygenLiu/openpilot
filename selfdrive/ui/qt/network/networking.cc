...
Networking::Networking(QWidget* parent, bool show_advanced) : QFrame(parent) {
  ...
  connectivityCheckTimer = new QTimer(this);
  connectivityCheckTimer->setSingleShot(true);
  connectivityCheckTimer->setInterval(10000);  // 10 second throttle
  connect(connectivityCheckTimer, &QTimer::timeout, this, &Networking::performConnectivityCheck);
...
void Networking::checkConnectivity() {
  if (connectivityCheckTimer->isActive()) {
    connectivityCheckPending = true;
    return;
  }
  performConnectivityCheck();
}

void Networking::performConnectivityCheck() {
  // GitHub connectivity check for sidebar CONNECT status
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    // Start throttle timer and perform pending check if needed
    connectivityCheckTimer->start();
    if (connectivityCheckPending) {
      connectivityCheckPending = false;
    }
  });
}
